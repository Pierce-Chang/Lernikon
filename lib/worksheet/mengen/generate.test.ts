...
describe("mengenConfigSchema", () => {
  ...
  it("rejects showSolutions as non-boolean", () => {
    const result = mengenConfigSchema.safeParse({ ...BASE, showSolutions: "yes" });
    expect(result.success).toBe(false);
  });
  ...
  it("accepts valid config with defaults", () => {
    ...
    expect(result.data.showSolutions).toBe(true);
  });
});

describe("generateMengen", () => {
  it("returns exactly count tasks for range 1-5", () => {
    const sheet = generateMengen({ ...BASE, range: "1-5", count: 6, showSolutions: true }, 1);
    expect(sheet.tasks).toHaveLength(6);
  });

  it("returns exactly count tasks for range 1-10", () => {
    const sheet = generateMengen({ ...BASE, range: "1-10", count: 18, showSolutions: false }, 2);
    expect(sheet.tasks).toHaveLength(18);
  });

  it("all quantities are in [1..5] for range 1-5", () => {
    const sheet = generateMengen({ ...BASE, range: "1-5", count: 6, showSolutions: true }, 3);
  ...
  it("all quantities are in [1..10] for range 1-10", () => {
    const sheet = generateMengen({ ...BASE, range: "1-10", count: 18, showSolutions: true }, 4);
    sheet.tasks.forEach((t) => {
      expect(t.quantity).toBeGreaterThanOrEqual(1);
  ...
  it("has no duplicate (shape, quantity) pairs", () => {
    const sheet = generateMengen({ ...BASE, range: "1-10", count: 18, showSolutions: true }, 5);
    const keys = sheet.tasks.map((t) => `${t.shape}|${t.quantity}`);
    expect(new Set(keys).size).toBe(keys.length);
  });

  it("is deterministic: same seed produces identical output", () => {
    const cfg = { ...BASE, range: "1-10" as const, count: 12 as const, showSolutions: true };
  ...
  it("uses more than 1 shape for count >= 8", () => {
    const sheet = generateMengen({ ...BASE, range: "1-10", count: 12, showSolutions: true }, 7);
    const shapes = new Set(sheet.tasks.map((t) => t.shape));
    expect(shapes.size).toBeGreaterThan(1);
  });

  it("seed is returned in the output", () => {
    const sheet = generateMengen({ ...BASE, range: "1-10", count: 6, showSolutions: true }, 99);
