...
- **Kid-Display-Schrift = Playwrite DE Grund.** Ziffern in Aufgaben/Lösungen, Buchstaben/Wörter in Schreib-Übungen → `fontFamily: "PlaywriteDEGrund"`. Helvetica nur für Header, Anweisungs-Zeile, Brand-Lockup, Footer/Watermark und Eltern-Vorleseblätter (z. B. Diktat Seite 1). SAS bleibt reserviert für den Schreibschrift-Toggle in Tracing-Topics. Nie `fontWeight: "bold"` auf PlaywriteDE*-Familien.
- **PlaywriteDEGrund hat einen Combining-Mark-Bug bei Umlauten.** Multi-Char-Strings mit ä/ö/ü/ß/Ä/Ö/Ü durch normales `<Text fontFamily="PlaywriteDEGrund">` rendern die Diakritika versetzt oder gar nicht ("Schüler" wird zu "Schuler"). Reine ASCII-Strings und einzelne Glyphen funktionieren. Workaround: derselbe fontkit-Outline-Trick wie für SAS — `font.layout(text)` → glyph runs + positions → `<Svg><Path d={glyph.path.toSVG()} transform="matrix(...)" />`. Referenz: `OutlinedGrundText` in `lib/worksheet/faelle/pdf.tsx` (Druckschrift, deutsche Sätze) und `OutlinedGhostWord` in `lib/worksheet/woerter-abschreiben/pdf.tsx` (Schreibschrift SAS). Bei reinen Zahlen-Topics (mengen/marienkaefer/multiplikation/...) trifft der Bug nicht zu — Ziffern sind nicht betroffen.
