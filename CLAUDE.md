...
- **Kid-Display-Schrift = Playwrite DE Grund.** Ziffern in Aufgaben/Lösungen, Buchstaben/Wörter in Schreib-Übungen → `fontFamily: "PlaywriteDEGrund"`. Helvetica nur für Header, Anweisungs-Zeile, Brand-Lockup, Footer/Watermark und Eltern-Vorleseblätter (z. B. Diktat Seite 1). SAS bleibt reserviert für den Schreibschrift-Toggle in Tracing-Topics; Grund hat den shaping-bug NICHT, multi-char `<Text>` rendert sauber (Referenz: `lib/worksheet/woerter-abschreiben/pdf.tsx` Druckschrift-Mode). Nie `fontWeight: "bold"` auf PlaywriteDE*-Familien.
