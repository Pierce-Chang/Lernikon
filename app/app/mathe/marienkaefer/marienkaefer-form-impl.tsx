...
import { COUNT_OPTIONS, type CountOption } from "@/lib/worksheet/marienkaefer/config";
...
interface MarienkaeferSettings {
  count: CountOption;
}

const DEFAULT_SETTINGS: MarienkaeferSettings = {
  count: 10,
};
...
export const MarienkaeferFormImpl = ({
  childId,
  quota,
}: {
  childId: string;
  ...
  const [settings, setSettings] = useLocalSettings<MarienkaeferSettings>(
    "lernikon.settings.mathe-marienkaefer",
    DEFAULT_SETTINGS,
  );
  const { count } = settings;
  const update = <K extends keyof MarienkaeferSettings>(
    key: K,
    nextValue: MarienkaeferSettings[K],
  ...
  const onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      capture("worksheet_generated", {
        operation: "marienkaefer-zaehlen",
        range_min: 1,
        range_max: 10,
        count,
      });
      const blob = await response.blob(),
        ...
        fallback = `Lernikon - Mathe - Marienkaefer zaehlen (${count} Aufgaben).pdf`,
