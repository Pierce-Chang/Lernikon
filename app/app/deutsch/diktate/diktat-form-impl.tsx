...
import { COUNT_OPTIONS, type DiktatCount } from "@/lib/worksheet/diktat/config";
...
export const DiktatFormImpl = ({
  ...
  const defaultSettings: DiktatSettings = { count: 8 };

  const [settings, setSettings] = useLocalSettings<DiktatSettings>(
    "lernikon.settings.deutsch-diktate",
    defaultSettings,
  );

  const { count } = settings;
  const update = <K extends keyof DiktatSettings>(
    key: K,
    nextValue: DiktatSettings[K],
  ...
  const onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      const blob = await response.blob(),
        ...
        fallback = `Lernikon - Deutsch - Diktat - Klasse 2.pdf`,
